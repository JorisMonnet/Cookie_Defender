...
void Map::moveMonster()
{
    ...
    for(Monster *monster : vectMonster){
        ...
        if(monster != nullptr){
            ...
            for(int i =0; i<4; i++)
                if(t[i].hasInRange(monster)){
                    //if(t[i].fireTimer != nullptr) delete t[i].fireTimer;

                    t[i].fireTimer = new QTimer(this);
                    t[i].fireTimer->start(t[i].shotSpeed*100); //because we defined the shotSpeed in s, and the start method need ms
                    connect(t[i].fireTimer, &QTimer::timeout, [=]() {
                        t[i].shot(monster);

                    });
                    if(monster->hp <= 0){
                        money += monster->moneyAccount;
                        //vectMonster.remove(i);
                        delete  monster;
                        //monster = nullptr;'
                    }
                }
